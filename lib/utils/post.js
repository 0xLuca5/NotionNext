...
export async function enrichPostsWordCountReadTime(
  posts,
  { from = 'list-wordcount', limit } = {}
) {
  const list = Array.isArray(posts) ? posts : []
  const target = Number.isFinite(limit) ? list.slice(0, limit) : list

  for (const post of target) {
    if (!post) continue
    if (post.password && post.password !== '') continue
    const hasWord = Number.isFinite(post.wordCount) && post.wordCount > 0
    const hasTime = Number.isFinite(post.readTime) && post.readTime > 0
    if (hasWord && hasTime) continue

    const blockMap = await getPostBlocks(post.id, from)
    if (blockMap?.block) {
      const content = Object.keys(blockMap.block).filter(
        key => blockMap.block[key]?.value?.parent_id === post.id
      )
      const pageContentText = getPageContentText({ ...post, content }, blockMap)
...
export async function processPostData(props, from) {
  ...
  if (allPosts && allPosts.length > 0) {
    ...
    // 同分类系列文章（给主题侧栏使用，避免保留 allPages 全量数据）
