...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  let content =
    ...
    pageRecordMap.block[configPageId]?.value?.value?.content
  ...
  const getBlockValue = blockId => {
    const raw = pageRecordMap.block?.[blockId]?.value
    if (!raw) return null
    return raw?.type ? raw : raw?.value || null
  }

  const isCollectionViewBlock = v =>
    v?.type === 'collection_view' || v?.type === 'collection_view_page'

  const findCollectionViewId = (ids, depth = 0) => {
    if (!Array.isArray(ids) || ids.length === 0) return null
    if (depth > 4) return null

    for (const id of ids) {
      const v = getBlockValue(id)
      if (isCollectionViewBlock(v)) {
        return id
      }
    }

    // 常见布局容器：column_list / column / toggle 等，继续递归 content
    for (const id of ids) {
      const v = getBlockValue(id)
      const children = v?.content
      if (Array.isArray(children) && children.length > 0) {
        const found = findCollectionViewId(children, depth + 1)
  ...
  // 找到PAGE文件中的database（兼容 column/布局包裹、collection_view_page）
  const configTableId = findCollectionViewId(content)
  ...
  const rawMetadata = getBlockValue(configTableId)
  ...
  const collectionRawValue = collectionEntry?.value
  const collection =
    collectionRawValue?.schema
      ? collectionRawValue
      : collectionRawValue?.value?.schema
        ? collectionRawValue.value
        : collectionRawValue?.value || collectionRawValue || {}
  ...
  for (let i = 0; i < pageIds.length; i++) {
    const id = pageIds[i]
    const value = normalizeBlockValue(block[id]?.value)
    if (!value) {
      continue
    }
    ...
    if (properties) {
      // 将表格中的字段映射成 英文
      const enableRaw = properties['启用'] ?? properties.Enable
      const enabled =
        enableRaw === true ||
        enableRaw === 'Yes' ||
        enableRaw === 'YES' ||
